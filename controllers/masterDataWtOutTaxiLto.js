...
const {
  LTO_WITH_OUT_TAXI,
  DB_NAME,
  LTO_WITH_OUT_TAXI_SEQ_ID,
} = require("../constants/database");
const { getEpochTime, getNextSequence } = require("../lib/utils");

exports.masterDataWtOutTaxiLto = async (req,res) =>{
    ...
    try {
      if (req.body.action == "viewDetails") { 
        const taskLists =  await client.db(DB_NAME).collection(LTO_WITH_OUT_TAXI).aggregate([
          {
            $facet: {
              data: [
                { $project: { _id: 0 } },
